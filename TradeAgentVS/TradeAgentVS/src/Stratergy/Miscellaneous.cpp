...
void PrimeryAndSecondary::OpenJudge(CThostFtdcDepthMarketDataField const& pDepthMarketData)
{
...
#endif
	string lServerTime = (string)pDepthMarketData.TradingDay + " "+(string)pDepthMarketData.UpdateTime;
	if(pDepthMarketData.UpdateMillisec == 500)
	{
		lServerTime = lServerTime + ".500";
	}
	if( lPosDeltPrice > lBoll.mOutterUpperLine )	
	{
		/* condition 1 */
		mOpenCond = OPEN_COND1;
		logger.LogThisFast("[EVENT]: OPEN_PRICE_GOOD_COND1	ServerTime: " + lServerTime);
		tempStream.clear();
		tempStream.str("");
	...
	else if( lNegDeltPrice < lBoll.mOutterLowerLine )
	{
		...
		logger.LogThisFast("[EVENT]: OPEN_PRICE_GOOD_COND2	ServerTime: " + lServerTime);
		tempStream.clear();
		tempStream.str("");
...
bool PrimeryAndSecondary::StopLoseJudge(CThostFtdcDepthMarketDataField const& pDepthMarketData)
{
	...
	{	
		...
		{
			//使用绝对浮亏来止损
			logger.LogThisFast("[EVENT]: MUST_STOP (from estimate profit)	ServerTime: " + lServerTime);
			tempStream.clear();
			tempStream.str("");
			if(BUY_SCND_SELL_PRIM == mTradeDir)
			{
...
bool PrimeryAndSecondary::StopWinJudge(CThostFtdcDepthMarketDataField const& pDepthMarketData)
{
	...
	if(mStateMachine.GetState() == PENDING_STATE)
	{
		...
		if(OPEN_COND1 == mOpenCond)
		{
			...
			{
				logger.LogThisFast("[EVENT]: CLOSE_PRICE_GOOD_COND1	ServerTime: " + lServerTime);
				tempStream.clear();
				tempStream.str("");
		...
		else if(OPEN_COND2 == mOpenCond)
		{
			...
			{
				logger.LogThisFast("[EVENT]: CLOSE_PRICE_GOOD_COND2	ServerTime: " + lServerTime);
				tempStream.clear();
				tempStream.str("");
		...
		{
			//使用绝对浮亏来止损
			logger.LogThisFast("[EVENT]: CLOSE_PRICE_GOOD (from estimate profit)	ServerTime: " + lServerTime);
			tempStream.clear();
			tempStream.str("");
			if(BUY_SCND_SELL_PRIM == mTradeDir)
			{
...
void PrimeryAndSecondary::LogBollData()
{
	...
	tempStream\
				...
				<<tempData.mInnerUpperLine<<"	"\
				<<tempData.mMidLine<<"	"\
				<<tempData.mInnerLowerLine<<"	"\
				<<tempData.mOutterLowerLine<<"	"\
				<<lPrim.volume<<"	"\
				<<lScnd.volume<<"	";
	if(mStateMachine.GetState() == PENDING_STATE)
	{
...
void PrimeryAndSecondary::StopOpenJudge(CThostFtdcDepthMarketDataField const& pDepthMarketData)
{
	...
	if(OPEN_COND1 == mOpenCond)
	{
		...
		{
			logger.LogThisFast("[EVENT]: OPEN_PRICE_BAD	ServerTime: " + lServerTime);
			tempStream.clear();
			tempStream.str("");
	...
	{
		logger.LogThisFast("[FATAL ERROR]: ILLEGAL OPEN COND	ServerTime: " + lServerTime);
	}
...
bool PrimeryAndSecondary::IsDataAligned(void)
{
	const BasicMarketData& lPrim = primDataBuf[primBufIndex];
	const BasicMarketData& lScnd = scndDataBuf[scndBufIndex];
	string lPrimUpdateTime = (string)lPrim.updateTime;
	string lScndUpdateTime = (string)lScnd.updateTime;
	string lPrimTradingDay = (string)lPrim.tradingDay;
	string lScndTradingDay = (string)lScnd.tradingDay;
	if( (lPrimUpdateTime == lScndUpdateTime) && (lPrim.updateMillisec == lScnd.updateMillisec) && (lPrimTradingDay == lScndTradingDay) )
	{
		return true;
	}
	else

...
namespace SelectStock_UpLimit
{
    class Program
    {
        ...
        {
			StreamWriter areaWriter = new StreamWriter("./report/area.csv");
			StreamWriter conceptWriter = new StreamWriter("./report/concept.csv");
			StreamWriter industryWriter = new StreamWriter("./report/industry.csv");
            try
            {
                ...
                SectorInfo sectorA = new SectorInfo("全部A股", SectorAStocks.Count, SectorAStocks, SectorAStocks);
                ...
                foreach (string sector in areaList)
                {
                    ...
                    SectorInfo tempSector = new SectorInfo(sector, sectorA.SectorCount, limitedStocks, sectorStocks);
				...
				foreach (SectorInfo sector in sectorInfoList)
				{
					areaWriter.WriteLine(sector.SectorName + "	" + sector.SectorUpLimitCount + "	" + sector.SectorCount + "	"
										...
										+ (double)sector.SectorUpLimitCount/(double)upLmtInAllArea+"	"+ sectorA.SectorCount);
				}
